...
class TD3Agent:
    def __init__(self,
        ...
        # handle our AUVEnv reset signature
        obs0, _    = env.reset()
        state_dim  = obs0.shape[0]
        # decide discrete vs continuous
        if getattr(env, 'discrete_actions', False):
            # discrete: #actions = len(env.actions), no scaling
            self.discrete = True
            action_dim   = len(env.actions)
            max_action   = 1.0
        else:
            # continuous: expect a Box of shape (2,), range [-1,1]
            self.discrete = False
    ...
    def select_action(self, state):
        if self.discrete:
            # ε‐greedy over action indices
            if random.random() < self.epsilon:
                return random.randrange(self.n_actions)
            with torch.no_grad():
                st = torch.tensor(state, dtype=torch.float32, device=self.device).unsqueeze(0)
                qv = self.policy_net(st)        # shape (1,action_dim)
                return int(qv.argmax(dim=1).item())
        else:
            # continuous policy output in [-max_action, +max_action]
            st = torch.tensor(state, dtype=torch.float32, device=self.device).unsqueeze(0)
